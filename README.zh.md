...
[![tests](https://img.shields.io/badge/tests-122_passing-success?logo=bun&logoColor=white)](#测试)
...
| `simple.limit` | `unsafe.bindings` | 每`60`秒`120`次 | 单个账号在这段时间里可以发起的Gmail调用次数，按该账号的全部会话合计。 |
...
122个单元测试覆盖邮件构建（MIME嵌套、RFC 2047折行、RFC 2231文件名、CR/LF拒绝、base64换行）、跨字符集的正文提取、回复与转发组装、Google token的交换与刷新、登录允许名单，以及登录流程里的CSRF和state校验。
