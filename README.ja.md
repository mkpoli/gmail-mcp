...
- **スコープを絞っています。** `gmail.modify`で許可されるのは、閲覧・送信・ラベル操作・ゴミ箱への移動までです。完全削除と`gmail.settings.*`配下は含まれません。乗っ取り後によく使われる自動転送やフィルタの作成は、そもそも権限の外にあります。あわせて`userinfo.email`と`userinfo.profile`も要求します。どちらも読み取り専用で、どのアカウントがサインインしたかを許可リストとセッションの結び付けが知るために使うもので、メールには届きません。
